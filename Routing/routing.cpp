...
main (int argc, char *argv[])
{
  if (argc == 2)
    {
     double lon,lat, min = 100000.0;
      int index;
      char b;
      std::vector<osmium::Location> v;
      
      //std::cout << "Your Coordinates2: ";
      std::cin >> lon >> lat;
      osmium::Location locs(lon,lat); //kiinduló koordináta
      //osmium::Location loctemp();
     
	std::cin >> lon;
	std::cin >> b;
	std::cin >> lat;
	while (lon || lat) {
		osmium::Location t(lon, lat);
		v.push_back(t);
		std::cin >> lon >> b >> lat;
	}


	//osmium::Location locf(21.6321, 47.5271);
      /*21.6298707
	47.5297339
	...
	47.5313374*/
      ...
      for(int i = 0 ; i < v.size() ; i++) {
	      double distance;
	      osmium::Location locf(v[i].lon(), v[i].lat());
	      ...
	      node_locationsc (locations);
	      osmium::apply (buffer, node_locationsc);
	      osmium::apply (buffer, routec);
	      routec.set_path(locs,locf);
	      
	      PathType pathc(routec.get_path());
	      Graph gc(routec.get_graph());
	      NameMap nameMapc = boost::get(boost::vertex_name, gc);
	      PathType::reverse_iterator pathIteratorc = pathc.rbegin();
	      
	      if (pathIteratorc != pathc.rend()) {
	      osmium::Location locc = locations.get(nameMapc[boost::source(*pathIteratorc, gc)]);
	      distance = 0.0;
	      for(; pathIteratorc != pathc.rend(); ++pathIteratorc)
		{
		   locc = locations.get(nameMapc[boost::target(*pathIteratorc, gc)]);
		   //std::cout <<loc.lon() <<", "  <<loc.lat()<<"\n";
	     	distance+=boost::get( boost::edge_weight, gc, *pathIteratorc );
	...
	std::cout << "\nKoordináták: " << v[index].lon() << ", " << v[index].lat() << std::endl;*/
	//std::cout << "Koordináták: " << v[index] << std::endl;
	osmium::Location locf(v[index].lon(), v[index].lat());
	

	//osmium::Location locs(21.6279, 47.5206);
	//osmium::Location locf(21.6321, 47.5271);
      //*******************************
	  ...
	  // std::cout <<"<Nodes>\n";
	   std::cout <<loc.lon() <<", "  <<loc.lat()<<"\n";
		...
		for(; pathIterator != path.rend(); ++pathIterator)
		{
		   ...
		   std::cout <<loc.lon() <<", "  <<loc.lat()<<"\n";
	     distance+=boost::get( boost::edge_weight, g, *pathIterator );
		}
